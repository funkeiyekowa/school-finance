...
75. `fix_student_clear_must_change_password.sql` — run after fix_teacher_login_and_password_change.sql (#47) and report_card_and_portals_migration.sql (#35), both already above. Extends clear_must_change_password() to also clear students.must_change_password for the caller's own linked student row. Fixes the student "Set Your New Password" screen that never dismissed (the direct table update was RLS-blocked). SECURITY DEFINER, scoped to auth.uid(); function replacement only, no data changes.
76. `fix_timetable_role_scoped_access.sql` — run after rls_role_scoped_access.sql (#50) and saas_foundation.sql, both already above. Adds is_non_teacher_staff(), is_assigned_to_class(p_class), is_my_current_class(p_class), get_my_current_class_id(), and rewrites timetable_entries' read policy to scope it to the caller's authorized class(es): privileged staff see everything, a teacher sees the union of their class-teacher and subject-teacher assignments (from teacher_assignments), and a student/parent sees only the student's own current class (resolved server-side via student_current_class_id()/my_linked_student_ids(), never a client-supplied id). Writes stay staff-only. Policy + function changes only, no data changes.
77. `20260906190000_timetable_setup_settings.sql` — run after fix_timetable_role_scoped_access.sql (#76) and saas_foundation.sql (for is_org_admin/is_org_member), both already above. Adds timetable_settings (one row per organisation: field visibility, title/footer text, paper size/orientation, layout density, font size, cell padding, borders, colours) and get_my_timetable_settings(). RLS: any org member can read (the print page needs it for every role), only org admins can write. Presentation only — never touches classes/subjects/periods/timetable_entries or who may see which class's timetable.
