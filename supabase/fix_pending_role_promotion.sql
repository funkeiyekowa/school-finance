-- ============================================================
-- FIX: Promote profiles stuck on role='pending' + stop new users
--      landing at 'pending' going forward.
--
-- v2 — the first version referenced staff_members.profile_id, which
--      doesn't exist. staff_members is joined to auth by email, and
--      teacher membership is inferred from teacher_assignments.user_id.
--
-- Root cause of the bug:
--   handle_new_user (schema.sql) sets every new profiles row to
--   role='pending', active=false. Downstream provisioners then run
--   `UPDATE profiles SET role = COALESCE(role, 'parent')`, which
--   preserves 'pending' because it is NOT NULL. Result: parent /
--   student rows land in the admin's pending list forever until
--   someone manually flips them.
--
-- This migration:
--   1. Backfill: promote every existing 'pending' profile by
--      inferring the true role from:
--        - students.profile_id       -> student
--        - parent_profiles.profile_id -> parent
--        - teacher_assignments.user_id OR staff_members.email
--                                     -> teacher
--        - org_memberships.role in (owner, admin) -> admin
--      Also sets profiles.active=true and flips the approval columns.
--   2. Adds a reusable helper `promote_pending_profile(uuid)`.
--   3. Rewrites auto_provision_parent so future INSERT/UPDATE paths
--      overwrite 'pending' instead of preserving it, and also flip
--      profiles.active=true.
--   4. Best-effort in-place patch of auto_provision_student to use
--      the same NULLIF pattern.
--
-- Idempotent. Safe to re-run.
-- ============================================================

-- ---------------------------------------------------------------------
-- 1. BACKFILL — promote every existing 'pending' profile.
-- ---------------------------------------------------------------------
DO $$
DECLARE
  r RECORD;
...
BEGIN
  ...
  LOOP
    ...
    -- teacher via teacher_assignments.user_id?
    IF v_role IS NULL
       AND EXISTS (SELECT 1 FROM public.teacher_assignments WHERE user_id = r.id) THEN
      v_role := 'teacher';
    END IF;

    -- teacher/staff via staff_members.email?
    IF v_role IS NULL
       AND r.email IS NOT NULL
       AND EXISTS (SELECT 1 FROM public.staff_members
                    WHERE LOWER(COALESCE(email,'')) = LOWER(r.email)) THEN
      v_role := 'teacher';
    END IF;
    ...
    IF v_role IS NOT NULL THEN
      ...
      RAISE NOTICE 'promoted % (%) -> %', r.email, r.id, v_role;
    END IF;
  END LOOP;
END $$;

-- ---------------------------------------------------------------------
-- 2. Reusable helper — safe to call from the app after login.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 3. Rewrite auto_provision_parent to overwrite 'pending' + flip active
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    IF v_old_email = v_email THEN RETURN NEW; END IF;
  END IF;
  ...
  -- Overwrite 'pending' explicitly. Keep any real prior role.
...
-- ---------------------------------------------------------------------
-- 4. Best-effort in-place patch of auto_provision_student
-- ---------------------------------------------------------------------
DO $$
DECLARE
  v_body text;
