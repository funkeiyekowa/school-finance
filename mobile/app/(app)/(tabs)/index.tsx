...
import { fetchClasses, type ClassRow } from "@/lib/attendance-service";
...
const roleTiles: Record<string, Tile[]> = {
  ...
  teacher: [{ icon: "▣", label: "My classes", detail: "Live classes you are authorised to capture attendance for." }, { icon: "✓", label: "Take attendance", detail: "Mark today's register for your classes.", href: "/(app)/(tabs)/attendance" }, { icon: "★", label: "Assessments", detail: "Open published exams and assessment results.", href: "/(app)/(tabs)/exams" }],
  staff: [{ icon: "▣", label: "School workspace", detail: "Live school classes and authorised daily workflows." }, { icon: "✓", label: "Take attendance", detail: "Mark the register for a class.", href: "/(app)/(tabs)/attendance" }, { icon: "✉", label: "Messages", detail: "Your school conversations.", href: "/(app)/(tabs)/messages" }],
  admin: [{ icon: "▣", label: "School overview", detail: "Live school classes and authorised daily workflows." }, { icon: "✓", label: "Take attendance", detail: "Mark the register for any class.", href: "/(app)/(tabs)/attendance" }, { icon: "₦", label: "Finance", detail: "Authorised finance tools will appear here." }]
};
...
export default function DashboardScreen() {
  ...
  useEffect(() => {
    if (!identity || (identity.role !== "teacher" && identity.role !== "staff" && identity.role !== "admin")) return;
    setClassesLoading(true);
    fetchClasses(identity.role, identity.userId)
      .then(setClasses)
      .catch((error: unknown) => setClassesError(error instanceof Error ? error.message : "Could not load authorised classes."))
