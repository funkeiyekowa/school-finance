...
`.env.local`; on Vercel they must be set in the **dashboard**
(Project → Settings → Environment Variables) for the target environment. **`.env.local` is
gitignored and is never uploaded to Vercel** — it only affects local builds, so fixing keys
there does nothing for a Vercel build. If `NEXT_PUBLIC_SUPABASE_ANON_KEY` is absent from the
build environment, the browser client (`src/lib/supabase/client.ts`) throws during static
prerender and the build fails (this is exactly what happened — see §9). Non-`NEXT_PUBLIC_`
vars (e.g. `SUPABASE_SERVICE_ROLE_KEY`) are **not** inlined and are read at **runtime**; they
also live in the dashboard. `vercel.json` intentionally carries **no** env vars.
