...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // Supabase intentionally avoids revealing whether an identity exists.
      // Preserve that privacy boundary: never call an account-existence RPC
      // and never vary the browser response based on the provider result.
      const { error } = await supabase.auth.resetPasswordForEmail(target, {
        redirectTo: `${window.location.origin}/auth/reset-password`,
      });
      if (error) {
        // Operational detail stays out of the anonymous UI. Server/provider
        // monitoring should be used for delivery diagnostics.
        console.warn("Password reset request was not accepted by the auth provider.");
      }
    } catch {
      console.warn("Password reset request could not reach the auth provider.");
    } finally {
      setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-[#F7F5F0] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <p className="text-gray-500 text-sm mb-6">Enter your email to request a secure reset link.</p>
        {message && (
          <div role="status" aria-live="polite" className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg text-green-800 text-sm">
            {message}
          </div>
        )}
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              required
              disabled={loading}
              className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A227] disabled:bg-gray-100"
            />
          </div>
